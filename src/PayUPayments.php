...
use VerumConsilium\PayU\Api\PayUPaymentMethodType;
...
use VerumConsilium\PayU\Util\RequestPaymentsUtil;
...
class PayUPayments
{
     ...
     * @return string The response to the ping request sent
     */
     ...
     * @param string $lang language of request see SupportedLanguages class
     * @return string The payment method list
     * @throws PayUException
     ...
     * @param string $lang language of request see SupportedLanguages class
     * @return string The payment method information
     * @throws PayUException
     * @throws InvalidArgumentException
     */
     ...
     * @param string $lang language of request see SupportedLanguages class
     *
     * @return string The bank list information
     * @throws PayUException
     * @throws InvalidArgumentException
     *
     */
    public static function getPSEBanks($parameters, $lang = null)
    {
        ...
        $request = RequestPaymentsUtil::buildBankListRequest($paymentCountry, $lang);
        $payUHttpRequestInfo = new PayUHttpRequestInfo(Environment::PAYMENTS_API, RequestMethod::POST);
        return PayUApiServiceUtil::sendRequest($request, $payUHttpRequestInfo);
    }
    ...
    public static function doPayment($parameters, $transactionType, $lang)
    {
        ...
        if ($paymentMethodParameter != null) {
            ...
            } elseif (array_key_exists(PayUParameters::CREDIT_CARD_NUMBER, $parameters)) {
            ...
            } elseif ($paymentMethod != null && (PayUPaymentMethodType::CASH == $paymentMethod->type)) {
                $requiredCash = array(
                        PayUParameters::PAYER_NAME,
            ...
            } elseif ("BOLETO_BANCARIO" == $paymentMethodParameter) {
            ...
            } elseif ("PSE" == $paymentMethodParameter) {
                $requiredPSE = array(
                        PayUParameters::REFERENCE_CODE,
                        PayUParameters::DESCRIPTION,
                        PayUParameters::CURRENCY,
            ...
            } elseif ($paymentMethod != null && ($paymentMethod->type == PayUPaymentMethodType::CREDIT_CARD)) {
    ...
    /**
     * Process a transaction already authorized
     *
     * @param array $parameters The parameters to be sent to the server
     * @param transactionType
     *            The type of the payment transaction
     ...
     *
     * @param array $parameters to build the request
     * @param string $lang language of request see SupportedLanguages class
     ...
     * @return boolean "hit the value for processWithoutCvv2 parameter, if the parameter doesn't exist in the array or
     * it has a invalid boolean value returns false;
     */
